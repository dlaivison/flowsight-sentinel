...
function PostCard({ post, onAlarmClick, onJustify }) {
  const sc      = STATUS[post.status] || STATUS.covered
  const isAlarm = post.status === 'alarm'
  const guards  = post.guards || []

  // Segundos de ausência do POSTO
  const [localSecs, setLocalSecs] = useState(post.absence_seconds || 0)

  // Segundos de ausência por VIGILANTE (mapa guard_id → seconds)
  const [guardSecs, setGuardSecs] = useState(() =>
    Object.fromEntries(guards.map(g => [g.guard_id, g.absence_seconds ?? 0]))
  )

  const [justifId,   setJustifId]   = useState(null)
  const [cancelling, setCancelling] = useState(false)

  // Sincroniza localSecs quando status do posto muda
  useEffect(() => {
    setLocalSecs(post.absence_seconds || 0)
  }, [post.status])

  // Para justified: mantém sincronizado sem piscar
  useEffect(() => {
    if (post.status !== 'justified') return
    setLocalSecs(s => {
      const srv = post.absence_seconds || 0
      return Math.abs(s - srv) > 3 ? srv : s
    })
  }, [post.absence_seconds])

  // Timer do POSTO — cresce ou decresce conforme status
  useEffect(() => {
    if (post.status === 'justified') {
      const t = setInterval(() => setLocalSecs(s => s > 0 ? s - 1 : 0), 1000)
      return () => clearInterval(t)
    }
    if (post.status === 'warning' || post.status === 'alarm') {
  ...
  // Sincroniza guardSecs quando servidor manda novos dados (detecção)
  useEffect(() => {
    setGuardSecs(prev => {
      const next = { ...prev }
      guards.forEach(g => {
        const srv = g.absence_seconds ?? 0
        // Reseta se servidor mandou valor menor (detecção recente)
        if (srv < (next[g.guard_id] ?? 999)) next[g.guard_id] = srv
      })
      return next
    })
  }, [post.guards])

  // Timer dos VIGILANTES — todos crescem a cada segundo
  useEffect(() => {
    const t = setInterval(() => {
      setGuardSecs(prev => {
        const next = { ...prev }
        for (const id in next) next[id] = (next[id] || 0) + 1
        return next
      })
    }, 1000)
    return () => clearInterval(t)
  ...
  const cancelJustify = async (e) => {
    ...
    if (!justifId) return
    setCancelling(true)
    try { await resolveJustification(justifId) } catch(_) {}
    finally { setCancelling(false) }
  }

  const warnT     = post.warning_threshold_seconds || 40
  const alarmT    = post.absence_threshold_seconds || 60
  const secsToAlarm = post.status === 'warning'
    ? Math.max(0, alarmT - localSecs)
    : null
  ...
  return (
    <div
      ...
      {/* Vigilantes com LED individual */}
      {guards.length > 0 && (
        <div style={{ display:'flex', flexWrap:'wrap', gap:5, marginBottom:10 }}>
          {guards.map(g => {
            const absSec   = guardSecs[g.guard_id] ?? g.absence_seconds ?? null
            const ledColor = absSec == null ? '#8B949E'
              : absSec < warnT  ? '#00D084'
              : absSec < alarmT ? '#F0A500'
              : '#FF4444'
            const ledLabel = absSec == null ? '—'
              : absSec < warnT  ? 'Presente'
              : absSec < alarmT ? 'Atenção'
              : 'Ausente'
            return (
              <div key={g.guard_id}
                title={`${g.guard_name} — ${ledLabel} (${absSec ?? '?'}s)`}
                style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 8px', background:'var(--bg-primary)', border:`1px solid ${ledColor}44`, borderRadius:'var(--radius-sm)' }}>
                <div style={{ width:7, height:7, borderRadius:'50%', background:ledColor, flexShrink:0, boxShadow:`0 0 5px ${ledColor}88` }}/>
                <div style={{ width:20, height:20, borderRadius:'50%', background:`${ledColor}22`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:8, fontWeight:700, color:ledColor, overflow:'hidden', flexShrink:0 }}>
      ...
      {/* Atenção — contador regressivo */}
      {post.status === 'warning' && secsToAlarm !== null && (
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', fontSize:11, color:'#F0A500', marginBottom:8, padding:'6px 8px', background:'rgba(240,165,0,0.08)', borderRadius:'var(--radius-sm)', border:'1px solid rgba(240,165,0,0.2)' }}>
          <span>⚠ Atenção — ausência detectada</span>
          <span style={{ fontFamily:'monospace', fontWeight:700, fontSize:13 }}>{secsToAlarm}s para alarme</span>
        </div>
      )}

      {/* Botão justificar */}
      {(post.status === 'alarm' || post.status === 'warning' || post.status === 'covered') && (
        <button onClick={e => { e.stopPropagation(); onJustify(post) }}
      ...
      {post.status === 'justified' && (
        <>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', fontSize:11, color:'#58A6FF', marginBottom:6, padding:'6px 8px', background:'rgba(88,166,255,0.08)', borderRadius:'var(--radius-sm)' }}>
            <span>⏸ Ausência justificada</span>
            <span style={{ fontFamily:'monospace', fontWeight:700, fontSize:13 }}>⏱ {countdown(localSecs)}</span>
          </div>
          <button onClick={cancelJustify} disabled={cancelling||!justifId}
            style={{ width:'100%', padding:'6px', borderRadius:'var(--radius-sm)', border:'1px solid rgba(0,208,132,0.3)', background:'rgba(0,208,132,0.08)', color:'#00D084', fontSize:11, fontWeight:600, cursor:'pointer', marginBottom:8 }}>
            {cancelling ? 'Encerrando...' : '✓ Vigilante retornou — Encerrar'}
          </button>
        </>
      )}
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', fontSize:11, color:'var(--text-muted)' }}>
        ...
        {post.status === 'warning' && (
          <span style={{ fontWeight:700, color:sc.color }}>{secsToAlarm}s para alarme</span>
        )}
        {post.status === 'alarm' && (
          <span style={{ fontWeight:700, color:sc.color }}>{localSecs}s</span>
        )}
      </div>

      {/* Barra de progresso */}
      {(post.status === 'warning' || post.status === 'alarm') && (
        <div style={{ marginTop:8, height:2, borderRadius:1, background:'var(--bg-hover)' }}>
          <div style={{ height:'100%', borderRadius:1, background:sc.color, width:`${Math.min(100,(localSecs/alarmT)*100)}%`, transition:'width .5s' }}/>
        </div>
      )}
    </div>
  )
...
export default function Operation() {
  const { guards: posts, openAlarmModal } = useFlowStore()
    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', background:'var(--bg-primary)' }}>
      ...
      <div style={{ padding:'16px 24px', borderBottom:'1px solid var(--border-subtle)', background:'var(--bg-secondary)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div style={{ fontSize:12, color:'var(--text-muted)' }}>{format(now, "dd/MM/yyyy · HH:mm", { locale: ptBR })}</div>
        </div>
      </div>
        ...
        {Object.entries(floors).map(([floor, floorPosts]) => (
          <div key={floor}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:12 }}>
              {floorPosts.map(p => <PostCard key={p.post_id} post={p} onAlarmClick={handleAlarmClick} onJustify={setJustifyPost}/>)}
            </div>
          </div>
        ))}
